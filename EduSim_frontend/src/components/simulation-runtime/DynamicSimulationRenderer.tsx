...
function drawArrow(ctx: CanvasRenderingContext2D, fromX: number, fromY: number, dx: number, dy: number, color: string) {
  const toX = fromX + dx;
  const toY = fromY + dy;
...
function renderCanvas(canvas: HTMLCanvasElement, snapshot: SimulationSnapshot | null) {
  ...
  if (simulationType === "projectile_motion" || simulationType === "gravity_system" || simulationType === "circular_motion") {
  ...
  if (simulationType === "gravity_system" || simulationType === "circular_motion") {
    const centerBody = snapshot.bodies.find((body) => body.role === "sun" || body.role === "anchor");
    const orbitBody = snapshot.bodies.find((body) => body.role === "planet" || body.role === "primary");
    if (centerBody && orbitBody) {
      ctx.strokeStyle = "rgba(96,165,250,0.15)";
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.arc(centerBody.position.x, centerBody.position.y, Number(orbitBody.properties.orbitRadius ?? 140), 0, Math.PI * 2);
      ctx.stroke();
    }
  }
  ...
  return (
    <div className="space-y-3 rounded-3xl border border-white/10 bg-black/30 p-4 backdrop-blur-xl">
      ...
      <div className="grid gap-3 md:grid-cols-2">
        <SceneControl label="Gravity" value={controls.gravity} min={0} max={20} step={0.1} onChange={(value) => onControlChange("gravity", value)} icon={<Gauge className="h-3.5 w-3.5" />} />
        <SceneControl label="Mass" value={controls.mass} min={0.5} max={20} step={0.1} onChange={(value) => onControlChange("mass", value)} icon={<Ruler className="h-3.5 w-3.5" />} />
        <SceneControl label="Force" value={controls.force} min={0} max={60} step={0.5} onChange={(value) => onControlChange("force", value)} icon={<Wind className="h-3.5 w-3.5" />} />
        <SceneControl label="Angle" value={controls.angle} min={0} max={90} step={0.5} onChange={(value) => onControlChange("angle", value)} icon={<ArrowRight className="h-3.5 w-3.5" />} />
        <SceneControl label="Velocity" value={controls.velocity} min={0} max={40} step={0.5} onChange={(value) => onControlChange("velocity", value)} icon={<Wind className="h-3.5 w-3.5" />} />
        <SceneControl label="Length" value={controls.length} min={60} max={260} step={1} onChange={(value) => onControlChange("length", value)} icon={<Ruler className="h-3.5 w-3.5" />} />
      </div>

      <div className="flex flex-wrap gap-2">
        <Button size="sm" variant="outline" onClick={onTogglePause} className="border-white/10 bg-white/5 text-white hover:bg-white/10">
          {isPaused ? <Play className="mr-2 h-4 w-4" /> : <Pause className="mr-2 h-4 w-4" />}
          {isPaused ? "Resume" : "Pause"}
        </Button>
        <Button size="sm" variant="outline" onClick={onReset} className="border-white/10 bg-white/5 text-white hover:bg-white/10">
...
function SceneSurface({ snapshot }: { snapshot: SimulationSnapshot | null }) {
  ...
  return <canvas ref={canvasRef} className="h-[540px] w-full rounded-3xl border border-white/10 bg-black/60" />;
}
...
export function DynamicSimulationRenderer({ dsl, formula, explanation, className }: Props) {
  ...
  const formulaEntry = useMemo(() => getFormulaEntry(dsl?.simulation_type || "newtons_second_law"), [dsl?.simulation_type]);
  ...
  if (!dsl) {
    return (
      <div className={cn("rounded-3xl border border-white/10 bg-black/40 p-8 text-center text-sm text-muted-foreground", className)}>
    ...
    <div className={cn("space-y-4", className)}>
      <div className="grid gap-4 xl:grid-cols-[1.3fr_0.9fr]">
        ...
        >
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs uppercase tracking-[0.3em] text-cyan-300/70">Dynamic Physics Runtime</p>
              <h2 className="mt-2 text-2xl font-semibold text-white">{dsl.topic}</h2>
              <p className="mt-2 text-sm text-slate-300">{explanation || formulaEntry.explanation}</p>
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs text-slate-300">
            {formulaEntry.formulas.map((entry) => (
              <span key={entry} className="rounded-full border border-white/10 bg-white/5 px-3 py-1">
                {entry}
              </span>
            ))}
          </div>
      ...
      <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-slate-300">
          ...
          <div className="mt-1 font-semibold text-white">{dsl.equations.slice(0, 2).join(" · ")}</div>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-slate-300">
          <div className="text-[11px] uppercase tracking-[0.3em] text-cyan-300/70">Gravity</div>
          <div className="mt-1 font-semibold text-white">{snapshot?.environment.gravity.toFixed(2) ?? dsl.environment.gravity.toFixed(2)} m/s²</div>
        </div>
        <div className="rounded-2xl border border-white/10 bg-white/5 p-3 text-xs text-slate-300">
          <div className="text-[11px] uppercase tracking-[0.3em] text-cyan-300/70">Bodies</div>
          <div className="mt-1 font-semibold text-white">{snapshot?.bodies.length ?? dsl.entities.length} active bodies</div>
        </div>
      </div>
    </div>
  );
