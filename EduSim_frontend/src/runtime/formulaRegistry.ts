...
const FORMULA_REGISTRY: Record<string, FormulaEntry> = {
  ...
  projectile_motion: {
    ...
    explanation: "Projectile motion separates into horizontal and vertical motion under constant gravity.",
    concepts: ["trajectory", "range", "launch angle", "gravity"],
  },
  pendulum: {
    formula: "T = 2π√(L/g)",
    formulas: ["T = 2π√(L/g)", "θ'' + (g/L) sin(θ) = 0"],
    explanation: "A pendulum oscillates due to gravity and its period depends on length and gravitational field.",
  ...
  gravity_system: {
    formula: "F = Gm₁m₂ / r²",
    formulas: ["F = Gm₁m₂ / r²"],
    explanation: "Bodies attract with gravitational force that falls off with the square of distance.",
  ...
  inclined_plane: {
    ...
    explanation: "On an incline, forces resolve into parallel and normal components with friction opposing motion.",
    concepts: ["incline", "friction", "normal force", "components"],
  },
  circular_motion: {
    formula: "F_c = mv² / r",
    formulas: ["F_c = mv² / r", "a_c = v² / r"],
    explanation: "Circular motion requires inward centripetal force to keep an object on a curved path.",
