...
function MySimulationsPage() {
  ...
  const handleExport = () => {
    ...
    window.open(
      simulationSynthesisService.getExportUrl(active.id),
      "_blank",
      "noopener,noreferrer",
    );
  };
        ...
        <div className="grid grid-cols-1 xl:grid-cols-[360px_1fr] gap-4">
          ...
          <div className="glass-strong rounded-3xl p-4 space-y-3">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              <div>
                ...
                <p className="text-xs text-muted-foreground">
                  {active?.description || "No simulation selected"}
                </p>
              </div>
            ...
            <div className="text-xs text-muted-foreground">
              Sources:{" "}
              {active?.sources?.map((s) => `${s.source} (p.${s.page})`).join(", ") ||
                "No textbook sources available"}
            </div>
