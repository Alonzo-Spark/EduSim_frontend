import { Sun, Moon, User, Menu, PanelLeft, Sparkles } from "lucide-react";
    ...
    <header className="sticky top-0 z-[60] w-full px-4 pt-4 pb-2 md:px-6">
      <div className="mx-auto flex items-center justify-between gap-4 backdrop-blur-xl bg-background/60 border border-border/50 px-4 py-3 rounded-[2rem] shadow-2xl">
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0">
          <button 
            onClick={toggleTheme}
            className="w-10 h-10 md:w-11 md:h-11 rounded-full flex items-center justify-center bg-secondary/50 border border-border hover:bg-secondary transition-all hover:scale-105 active:scale-95 group cursor-pointer"
          ...
          <div className="flex items-center pl-2 border-l border-border/50">
            <Link to="/profile" className="w-10 h-10 md:w-11 md:h-11 rounded-full bg-gradient-to-br from-[var(--neon-purple)] to-[var(--neon-blue)] flex items-center justify-center shadow-lg hover:scale-105 active:scale-95 transition-all">
