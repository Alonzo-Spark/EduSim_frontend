...
export const DebugDashboard: React.FC = () => {
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      setLogs((prev) =>
        [{ type, payload, time: new Date().toLocaleTimeString() }, ...prev].slice(0, 50),
      );
        ...
        <CardContent className="py-2 flex flex-col gap-3">
          <div className="grid grid-cols-2 gap-2">
            <div className="bg-slate-800 p-2 rounded-lg text-center">
              ...
              <div
                className={`text-lg font-bold ${health && health.stability < 80 ? "text-rose-400" : "text-emerald-400"}`}
              >
            ...
            <div className="bg-slate-800 p-2 rounded-lg text-center">
              ...
              <div className="text-lg font-bold text-sky-400">
                {Math.round(health?.performance || 0)}%
              </div>
            </div>
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="max-h-40 overflow-y-auto space-y-1 pr-1 scrollbar-hide">
              {logs.length === 0 && (
                <div className="text-xs text-slate-500 italic p-2">Waiting for simulation...</div>
              )}
              {logs.map((log, i) => (
                <div
                  key={i}
                  className="text-[10px] bg-slate-800/50 p-1.5 rounded flex justify-between items-start border border-slate-700/50"
                >
                  <span className="flex items-center gap-1">
                    {log.type === "sim_error" && (
                      <AlertTriangle className="w-3 h-3 text-rose-400" />
                    )}
                    {log.type === "sim_interaction" && (
