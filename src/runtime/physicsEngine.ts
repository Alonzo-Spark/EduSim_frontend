...
function createBodyFromEntity(
  entity: SimulationDSL["entities"][number],
  index: number,
...
export function buildRuntimeWorld(dsl: SimulationDSL): RuntimeWorld {
  ...
  const bodies = dsl.entities.map((entity, index) =>
    createBodyFromEntity(entity, index, dsl.simulation_type, environment),
  );
  ...
  if (dsl.simulation_type === "pendulum") {
    const bob = bodies[0];
    const pivot = toVector2(bob.properties.pivot as { x: number; y: number } | undefined, {
      x: environment.width * 0.52,
      y: 100,
    });
...
export class PhysicsEngine {
  ...
  private applyControlToBodies() {
    ...
    if (simulationType === "pendulum") {
      ...
      primary.mass = this.world.controls.mass;
      const pivot = toVector2(primary.properties.pivot as { x: number; y: number } | undefined, {
        x: this.world.environment.width * 0.52,
        y: 100,
      });
      const angle = this.world.controls.angle * (Math.PI / 180);
    ...
    if (simulationType === "circular_motion") {
      primary.mass = this.world.controls.mass;
      const center = toVector2(
        primary.properties.orbitCenter as { x: number; y: number } | undefined,
        { x: this.world.environment.width * 0.54, y: this.world.environment.height * 0.5 },
      );
  ...
  private updateGravityScene(dt: number) {
    ...
    const force =
      (this.world.environment.gravity * 1800 * sun.mass * planet.mass) / (distance * distance);
  ...
  private updateInclinedPlaneScene(dt: number) {
    ...
    if (!block) return;
    const angle =
      Number(block.properties.planeAngle ?? this.world.controls.angle) * (Math.PI / 180);
    const origin = toVector2(block.properties.trackOrigin as { x: number; y: number } | undefined, {
      x: this.world.environment.width * 0.18,
      y: this.world.environment.height * 0.76,
    });
    const acceleration =
      this.world.environment.gravity *
      (Math.sin(angle) - this.world.environment.friction * Math.cos(angle));
  ...
  private updateCircularScene(dt: number) {
    ...
    const angularSpeed = Number(
      body.properties.angularSpeed ?? this.world.controls.velocity / radius,
    );
