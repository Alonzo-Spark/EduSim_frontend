...
import { 
  ...
  Lightbulb, Compass, Award, Activity
} from 'lucide-react';
...
export const AssistedModePanel: React.FC<AssistedModePanelProps> = ({
  ...
  // Smart Context-Aware Physics Hints
  const getContextHint = (): { title: string; explanation: string; formula?: string } => {
    const hasRope = validationState.constraints.some(c => c.type === 'rope' || c.type === 'rope-chain');
    const hasSpring = validationState.constraints.some(c => c.type === 'spring');

    if (hasRope) {
      return {
        title: 'Simple Pendulum Oscillations',
        explanation: "Gravity provides the restoring force. Under Hooke's rules or linear pendulum approximations, the period T depends only on rope length L, not mass!",
        formula: 'T = 2π √(L / g)'
      };
    }
    if (hasSpring) {
      return {
        ...
        explanation: "Hooke's Law explains restoring force proportional to displacement. Stiff springs cause rapid oscillations; larger mass slows the system.",
        formula: 'F = -k · x'
      };
    }
    if (validationState.gravityMode === 'radial') {
      return {
        title: 'Keplerian Orbits & Centripetal Gravity',
        explanation: 'Planetary radial orbits depend on the gravitational constant G and central star mass. Tangent velocities prevent direct collapsing!',
        formula: 'v = √(G · M / r)'
      };
    }
    return {
      title: 'Newtonian Force Mechanics',
      explanation: 'Unbalanced forces generate acceleration. Check how adding restitution scales collision rebounds and alters linear momentum values.',
      formula: 'F = m · a'
    };
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
        ...
        {/* Real-time Scientific Insights Deck */}
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '6px', color: '#fbbf24', marginBottom: '8px' }}>
            <Lightbulb size={14} />
            <span style={{ fontSize: '11px', fontWeight: 800, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              Live Physics Tutor
            </span>
          </div>
          <div style={{ fontSize: '11.5px', fontWeight: 800, color: '#f8fafc', marginBottom: '4px' }}>
            {hint.title}
          </div>
